...
select ename, substr(hiredate,4,2) as "입사년도 4월"
from emp
...
select sysdate, to_date('2020-01-01','YYYY-MM-DD'), 
        trunc(sysdate-to_date('2020-01-01','YYYY-MM-DD'))
         --trunc(sysdate-to_date('2020-01-01','YYYY-MM-DD'),0)      -- 뒤에 소수점 자른다.
from dual
;
...
26. 관리자 수를 나열하시오.   (job에 있는 manager 말고, MGR말하는 거다.)      
...
28. 직급별 사원의 최저 급여를 출력하시오. 
    관리자를 알 수 없는 사원의      where mgr is not null
    최저 급여가 2000 미만인 그룹은 제외시키고   having min(sal) >=2000 ,   2000이상만 출력하겠다는 뜻.
...
decode(deptno, 10, '서울',
                ...
                30, '인천',
                40, '제주도') as "지역명",
                count(*) as "사원 수", round(avg(sal),0) as "평균급여"
from emp
group by deptno
;
