...
select min(avg)
from (select avg(sal) as avg           -- 인라인뷰
        from emp
        group by job)
;
...
53. 이름에 K가 포함된 사원과 같은 부서에서 일하는 
    사원의 사원 번호와 이름, deptno을 표시하시오.

select distinct deptno from emp where ename like '%K%';     - 10,30번 부서에 있다.
...
select deptno from dept where loc='DALLAS';        --20번 부서
...
from emp                                                                            
where mgr = (select empno from emp where ename='KING')      -- mgr이 king인 애들이다
;                                                           --job이 manager애들이 보고한다.
...
select e.deptno, empno, job         -- 2번 방법
from emp e, dept d
...
57. 평균 월급보다 많은 급여를 받고 
    ...
    사원의 사원 번호, 이름, 급여, deptno를 표시하시오.
    
select avg(sal) from emp;       --2073

select distinct deptno, ename from emp where ename like '%M%';
...
select distinct deptno from emp where job='MANAGER';        --10,20,30번 부서에 manager 다 있다.
